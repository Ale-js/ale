...
(function(window, document) {
    ...
    window.Ale = function(name, config) {
        if (typeof name !== "string" || !isRealObj(config)) {
            if (!isRealObj(name)) {
                ...
                return new aleLocalComponentGenerateObject(handleComponentCreate(name));
            }
        }
        ...
        componentList[name] = handleComponentCreate(config);
    }
    ...
    Ale.plugin = {};
    Ale.plugin.inject = function(fun) {
        if (typeof fun !== "function") {
    ...
    function executePluginInject(th, watch) {
    ...
    function firstRenderingDOM(i, data, methods, obj) {
        ...
        executePluginInject(i, obj.watch);
    }

    function updateDOM(data, methods, obj) {
        /* obj内部属性
            type
            template
            life
            ...
            if (rChildren[i] === undefined) {
                ...
                if (vNewChildren[i].nodeName === "#text") {
                    ...
                    if (rChildren[i].nodeName !== "#text") {
                        //不是
                        rChildren[i].parentNode.innerHTML = vNewChildren[i].nodeValue;
                    } else {
